...
// Extracted from what used to be the standalone dashboard/address/page.jsx
// (still available at that URL for back-compat/bookmarks — see that file,
// now just a thin wrapper around this) so the exact same address CRUD can
// also be embedded as a tab inside My Profile (dashboard/profile/page.jsx)
// — "Everything else will be under my profile" — without duplicating any
// of this logic.
//
// A mobile number is no longer required on the account profile itself
// (see dashboard/profile/page.jsx) to place an order, but it's still
// genuinely needed for delivery — the courier needs a way to reach
// whoever's receiving the order — so the requirement lives here instead,
// on the actual delivery address used for an order, where it belongs.
const FIELDS = [
...
export default function AddressBook({ showHeading = true }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        {showHeading ? <h1 className="section-heading text-2xl">Addresses</h1> : <div />}
        {!adding && (
